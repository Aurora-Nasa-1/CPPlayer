...
```jsonc
{
    ...
    // 可选：支持的 CPU 架构列表
    // 用于声明模块支持的 ABI，加载器会自动根据设备架构选择正确的 native library
    // 支持的值："arm64-v8a", "armeabi-v7a", "x86_64", "x86"
    // 省略时按单架构模块处理（向后兼容）
...
适用于需要同时支持 ARM64 和 ARM32 设备的 JNI/Binary 模块。
...
my-provider.zip
  ...
  └── lib/
      ...
      └── armeabi-v7a/
          └── libmybackend.so    # ARM32 共享库
```

**Binary Provider（多 ABI）：**
```
my-provider.zip
  ├── manifest.json
  └── lib/
...
cp target/aarch64-linux-android/release/libmybackend.so lib/arm64-v8a/
cp target/armv7-linux-androideabi/release/libmybackend.so lib/armeabi-v7a/
zip -r my-jni-provider.zip manifest.json lib/
```
